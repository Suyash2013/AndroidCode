...
function globalConfigFile() {
  const candidates = ["androidcode.jsonc", "androidcode.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "androidcode.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "androidcode.jsonc")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const name of ["androidcode", "opencode"]) {
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.androidcode/plugin(s)` or `.opencode/plugin(s)` are already local files, so ConfigPlugin.load
