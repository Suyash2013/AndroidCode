...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .androidcode/ or .opencode/ subdirectory too)
  const candidates = [path.join(baseDir, "androidcode.json"), path.join(baseDir, "androidcode.jsonc"), path.join(baseDir, "opencode.json"), path.join(baseDir, "opencode.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".androidcode", "androidcode.json"),
      path.join(baseDir, ".androidcode", "androidcode.jsonc"),
