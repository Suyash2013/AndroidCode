...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".opencode") || binDir.includes(".androidcode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using AndroidCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes("# androidcode") || content.includes(".opencode/bin") || content.includes(".androidcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# opencode" || trimmed === "# androidcode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opencode/bin") || trimmed.includes(".androidcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".opencode/bin") || trimmed.includes(".androidcode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".opencode") || trimmed.includes(".androidcode")))
    ) {
      continue
    }
