...
export type PatchDeps = {
  ...
  files: (dir: string, name: "opencode" | "androidcode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".androidcode")
}

function patchName(kind: Kind): "androidcode" | "tui" {
  if (kind === "server") return "androidcode"
  return "tui"
}
