...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  ...
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "androidcode.json"),
      JSON.stringify({
        $schema: "https://androidcode.dev/config.json",
        ...options.config,
      }),
    )
  }
  ...
  return Effect.gen(function* () {
    ...
    if (options?.config) {
      yield* Effect.promise(() =>
        fs.writeFile(
          path.join(dir, "androidcode.json"),
          JSON.stringify({ $schema: "https://androidcode.dev/config.json", ...options.config }),
        ),
      )
    }
